import type { CellValue, Point, RecognitionResult, Stroke } from "./types";

const SAMPLE_POINTS = 64;
const CONFIDENCE_THRESHOLD = 0.55;

/** Templates simplifiés (coordonnées normalisées 0–1) pour chiffres 1–9 */
const DIGIT_TEMPLATES: Record<number, Point[]> = {
  1: line(0.5, 0.08, 0.5, 0.92),
  2: path([
    [0.12, 0.18],
    [0.88, 0.18],
    [0.88, 0.48],
...
function path(coords: [number, number][]): Point[] {
  ...
  return resamplePoints(points, SAMPLE_POINTS);
}
...
export function recognizeDigit(strokes: Stroke[]): RecognitionResult {
  ...
  const candidate = resamplePoints(normalized, SAMPLE_POINTS);
  ...
  const confidence = Math.max(0, 1 - bestDistance / 0.45);
